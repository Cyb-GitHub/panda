...
export default {
	...
	data() {
		return {
			...
			status: true, // 滑动的状态，表示当前是展开还是关闭按钮的状态
	...
	methods: {
		...
		ontouchmove(val) {
			console.log()
			const move = val.changedTouches[0].clientX;
			if (this.scrollX && move < this.scrollX) {
				this.emitRightEvent()
			} else if (this.scrollX && move > this.scrollX) {
				this.emitLeftEvent()
			}
		...
		// 开始触摸
		touchstart(val) {
			this.scrollX = val.changedTouches[0].clientX;
			this.moveX = 0
			this.$nextTick(function() {
				this.moveX = 0
			})
